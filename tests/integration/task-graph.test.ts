...
describe('Task V2 持久化任务图', () => {
  ...
  it('创建任务后可读取 + listTasks 返回完整列表', async () => {
    const t1 = await createTask(opts, { subject: '运行测试', description: '跑全部 vitest' })
    expect(t1.id).toBe('1')
  ...
  it('updateTask 可改字段、改状态、加依赖', async () => {
    const t1 = await createTask(opts, { subject: '步骤A', description: 'first' })
    const t2 = await createTask(opts, { subject: '步骤B', description: 'second' })
  ...
  it('isReady：pending 且无未完成上游 = ready；其余 = not ready', async () => {
    const t1 = await createTask(opts, { subject: 'A', description: 'first' })
    const t2 = await createTask(opts, { subject: 'B', description: 'second' })
  ...
  it('resetTaskGraph 清空全部任务但 highwatermark 保留（id 不复用）', async () => {
    await createTask(opts, { subject: '1', description: 'd1' })
    await createTask(opts, { subject: '2', description: 'd2' })
    await createTask(opts, { subject: '3', description: 'd3' })
    ...
    const fresh = await createTask(opts, { subject: 'after-reset', description: 'd' })
    expect(fresh.id).toBe('4')
  })

  it('删除任务后 highwatermark 仍保留', async () => {
    const t1 = await createTask(opts, { subject: '会被删', description: 'd' })
    expect(t1.id).toBe('1')

    await deleteTask(opts, t1.id)
    const next = await createTask(opts, { subject: '新任务', description: 'd' })
    expect(next.id).toBe('2') // 不复用 '1'
  })

  it('createTask 与 updateTask 保留 metadata 字段', async () => {
    const t = await createTask(opts, {
      subject: '带元数据',
      description: '验证 metadata',
