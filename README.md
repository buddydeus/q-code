...
基于 AI SDK 的命令行 Agent 框架，支持工具调用、Plan Mode、TodoWrite、上下文自动压缩、会话持久化、跨对话项目记忆和 MCP 扩展。
...
| `todo_write` | 会话级任务清单，全量替换，全部完成自动清空 |
...
TodoWrite 是会话级任务清单，给模型一张“便签纸”跟踪多步骤任务。它不是持久化任务图，进程退出后清单会丢失；后续 Task V2 才负责磁盘持久化、依赖关系和多 agent 协作。
