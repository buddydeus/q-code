...
export interface AuditLogger {
  emit(event: AuditEventName, payload?: Record<string, unknown>, ctx?: AuditContext): void
  flush(): Promise<void>
}
...
export class NdjsonAuditLogger implements AuditLogger {
  ...
  constructor(options: AuditLoggerOptions = {}) {
    ...
    if (this.config.enabled) {
      ...
      cleanupExpiredAuditFiles(this.config.auditDir, this.config.retentionDays, this.now())
    }
    if (options.registerProcessHandlers !== false) registerAuditProcessFlush(this)
  }
  ...
  private async drain(): Promise<void> {
    while (this.queue.length > 0) {
      ...
      try {
        ...
        const filePath = await this.resolveWritePath(Buffer.byteLength(line, 'utf-8'), record.ts)
  ...
  private async resolveWritePath(nextBytes: number, timestamp: string): Promise<string> {
    const date = timestamp.slice(0, 10)
    let suffix = 0
    while (true) {
      const filePath = join(this.config.auditDir, formatAuditFileName(date, suffix))
...
export function getAuditLogger(): AuditLogger {
  singleton ??= getAuditConfig().enabled ? new NdjsonAuditLogger() : new NoopAuditLogger()
  return singleton
}
...
export function getAuditConfig(env: NodeJS.ProcessEnv = process.env): AuditConfig {
  const qCodeHome = env.Q_CODE_HOME?.trim() ? resolve(env.Q_CODE_HOME) : join(homedir(), '.q-code')
  return {
    enabled: !isFalseEnv(env.Q_CODE_AUDIT_ENABLED),
    auditDir: resolve(env.Q_CODE_AUDIT_DIR?.trim() || join(qCodeHome, 'logs')),
    retentionDays: getPositiveIntEnv(env.Q_CODE_AUDIT_RETENTION_DAYS, DEFAULT_RETENTION_DAYS),
    maxFileBytes: getPositiveIntEnv(env.Q_CODE_AUDIT_MAX_FILE_BYTES, DEFAULT_MAX_FILE_BYTES),
    maxQueueSize: getPositiveIntEnv(env.Q_CODE_AUDIT_MAX_QUEUE_SIZE, DEFAULT_MAX_QUEUE_SIZE),
    piiMode: env.Q_CODE_AUDIT_PII?.trim().toLowerCase() === 'full' ? 'full' : 'hash'
  }
}

export function createUserPromptPayload(text: string, piiMode = getAuditConfig().piiMode) {
...
export function createToolCallPayload(args: {
  ...
  if ((args.piiMode ?? getAuditConfig().piiMode) === 'full') payload.input = args.input
  return payload
}
...
export function createToolResultPayload(args: {
  ...
  const text = typeof args.output === 'string' ? args.output : safeStringify(args.output)
  const payload: Record<string, unknown> = {
    name: args.name,
    ok: args.ok,
  ...
  if ((args.piiMode ?? getAuditConfig().piiMode) === 'full') payload.output = args.output
  return payload
}

export function createMessageSummaryPayload(text: string): Record<string, unknown> {
  return {
    chars: text.length,
    sha256: sha256(text)
  }
...
function cleanupExpiredAuditFiles(auditDir: string, retentionDays: number, now: Date): void {
  if (retentionDays <= 0 || !existsSync(auditDir)) return
  const cutoff = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()) - retentionDays * 86400_000
...
function registerAuditProcessFlush(logger: AuditLogger): void {
...
function formatAuditFileName(date: string, suffix: number): string {
  return suffix === 0 ? `audit-${date}.ndjson` : `audit-${date}.${suffix}.ndjson`
}
...
function getPositiveIntEnv(env: NodeJS.ProcessEnv, name: string, fallback: number): number
function getPositiveIntEnv(
  ...
  const raw =
    typeof nameOrFallback === 'string'
      ...
      : envOrValue
  const fallback = typeof nameOrFallback === 'number' ? nameOrFallback : maybeFallback!
  const value = Number(String(raw ?? '').trim().replace(/_/g, ''))
