...
try {
  ...
  const formatted = formatMailboxAttachment([
    ...
    { from: 'frontend', text: 'pinged', timestamp: '2026-01-01T00:00:01Z', read: false, summary: 'ping' }
  ])
  ...
  const noActiveOk = await teamCreate.execute({ team_name: 'reviewers', description: 'parallel review' }, { cwd })
  assert(String(noActiveOk).includes('Team "reviewers" created'), 'TeamCreate succeeds when no team is active')
  ...
  assert(String(dupActive).startsWith('Error:'), 'TeamCreate refuses a second team while one is active')
  ...
  const leadSend = await sendMessage.execute(
    { to: 'security', message: 'check sql.ts' },
    { cwd }
  )
  ...
  const broadcast = await sendMessage.execute(
    { to: '*', message: 'wrap up please' },
    { cwd }
  )
  ...
  assert(perfInbox.some((m) => m.from === 'security'), 'inbox shows the teammate identity as sender')

  const unknownRecipient = await sendMessage.execute(
    { to: 'ghost', message: 'where are you' },
    { cwd }
  )
  assert(String(unknownRecipient).includes('no teammate named "ghost"'), 'unknown recipient rejected')
  ...
  assert(String(refused).includes('teammate(s) still active'), 'TeamDelete refuses while members are active')
  ...
  assert(String(missingPair).includes("'name' and 'team_name' must be used together"), 'unpaired name rejected')
  ...
  assert(String(nested).includes('nested teammate spawn rejected'), 'nested teammate spawn rejected')
  ...
  assert(String(launched).includes('<teammate_name>reviewer</teammate_name>'), 'launch result tags teammate')
  assert(
